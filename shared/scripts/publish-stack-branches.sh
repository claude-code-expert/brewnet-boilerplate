...
# Ensure we start on source branch
...
for stack in "${STACKS[@]}"; do
    ...
    # Save stack files to temp dir BEFORE switching branches
    # (branch switch + git clean removes the stacks/ directory)
    TMPDIR_STACK=$(mktemp -d)
    cp -R "$STACK_DIR/." "$TMPDIR_STACK/"

    VALIDATE_SH="$REPO_ROOT/shared/scripts/validate.sh"

    # Create orphan branch or checkout existing stack branch
    ...
    # Copy stack files from temp dir to root
    cp -R "$TMPDIR_STACK/." .
    rm -rf "$TMPDIR_STACK"
...
GITIGNORE
    ...
    # Stage and commit
    git add -A
    git commit -m "chore: publish $stack stack

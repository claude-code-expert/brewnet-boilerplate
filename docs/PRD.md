...
`brewnet create-app` 명령 하나로 **즉시 실행 가능한 풀스택 모노레포**를 생성한다. Git에서 템플릿을 shallow clone하고, 선택한 스택(Go/Rust/Java/Kotlin/Node/Python × 다양한 프레임워크)과 DB(PostgreSQL/MySQL/SQLite3)에 맞는 설정 파일을 적용한 뒤, `docker compose up` 한 번으로 Hello World API 서버 + 프론트엔드(React/Vue/Svelte) + DB가 동작한다.
...
│   ├── frontend-vue/                # Vue 3 + Vite 프론트엔드 템플릿
│   ├── frontend-svelte/             # SvelteKit 5 프론트엔드 템플릿
...
#### 6.16 Frontend — Vue 3 (`frontend-vue`)

Vue 3 Composition API + Vite + TypeScript. 기본 React 프론트엔드 대체용.

#### 6.17 Frontend — SvelteKit 5 (`frontend-svelte`)

SvelteKit 5 + `$state` runes. 기본 React 프론트엔드 대체용.

#### 6.18 API-Only Template (`docker-compose.api-only.yml.template`)

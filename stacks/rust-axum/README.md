...
Switch databases by changing `DB_DRIVER` in your `.env` file. The backend uses **SQLx AnyPool** to connect to any supported driver at runtime. The database pool is stored as `Option<AnyPool>` in Axum's state, allowing the server to start gracefully even when no database is available.

`.env` 파일에서 `DB_DRIVER`를 변경하여 데이터베이스를 전환합니다. 백엔드는 **SQLx AnyPool**을 사용하여 런타임에 지원되는 모든 드라이버에 연결합니다. 데이터베이스 풀은 Axum의 state에 `Option<AnyPool>`로 저장되어 데이터베이스가 없어도 서버가 정상적으로 시작됩니다.
...
| `STACK_LANG` | `rust-axum` | Stack identifier / 스택 식별자 |
...
├── backend/
│   ├── src/
│   │   ├── main.rs              # Server entry point, Axum router, CORS + State setup
│   │   │                        # 서버 진입점, Axum 라우터, CORS + State 설정
...
│   ├── Cargo.toml               # Dependencies: axum 0.8, tower-http 0.6, sqlx 0.8, tokio, chrono, serde

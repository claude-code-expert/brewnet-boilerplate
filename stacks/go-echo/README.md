...
|--------|------|----------|---------------------|
| GET | `/` | `{"service":"echo-backend","status":"running","message":"..."}` | Service info / 서비스 정보 |
| GET | `/health` | `{"status":"ok","timestamp":"...","db_connected":bool}` | Healthcheck / 헬스체크 |
| GET | `/api/hello` | `{"message":"Hello from Echo!","lang":"go","version":"..."}` | Hello API |
